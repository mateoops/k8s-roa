...
func (KubernetesHandler *KubernetesHandler) ListPods(ignoredNamespaces []string) []prometheus.PodMetrics {
	...
	for _, ns := range namespaces.Items {
		// ignore namespace if added to ignored list in config
		if !slices.Contains(ignoredNamespaces, ns.Name) {
...
func (kubernetesHandler *KubernetesHandler) GetPodUsageMetrics(podName, namespace string) []prometheus.ContainerUsageMetrics {
	...
	for _, containerMetrics := range metrics.Containers {
		// CPU usage measured in millicpu (1000 millicpu = 1cpu)
		cpuUsage := containerMetrics.Usage.Cpu().AsDec().UnscaledBig().Int64() / 1024 / 1024
		// Memory usage measured in MB
		memoryUsage := containerMetrics.Usage.Memory().MilliValue() / 1024 / 1024 / 1024

		containersUsageMetricsList = append(containersUsageMetricsList, prometheus.ContainerUsageMetrics{
			CpuUsage:    cpuUsage,
			MemoryUsage: memoryUsage,
			Container:   prometheus.ContainerMetrics{Name: containerMetrics.Name, Pod: prometheus.PodMetrics{Name: podName, Namespace: namespace}},
		})
	}
